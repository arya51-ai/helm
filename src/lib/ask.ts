...
function findBusiness(q: string, businesses: Business[]): Business | undefined {
  ...
  if (/\b(smoke|vape|tobacco)\b/.test(q)) return businesses.find((b) => /havana/i.test(b.name));
...
export function answerQuestion(raw: string, ctx: AskContext): AskAnswer {
  ...
  // Fallback
  return {
    text: `I can tell you about sales, profit, net worth, your portfolio, or where to put cash. Try "How's Havana this week?" or "Where should I put my cash?"`,
  };
}
