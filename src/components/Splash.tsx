...
export function Splash({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div
      className={cx(
        "absolute inset-0 z-[80] flex flex-col items-center justify-center bg-[#0a0b10] transition-opacity duration-500",
        leaving ? "opacity-0" : "opacity-100",
      )}
    >
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-1/2 top-1/3 h-64 w-64 -translate-x-1/2 rounded-full bg-violet-600/25 blur-[100px]" />
      </div>
      <div className="animate-fade-up flex flex-col items-center">
        <HelmTile size={84} rx={28} className="shadow-2xl shadow-violet-900/40" />
