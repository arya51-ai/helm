...
function Wheel({ stroke, dot }: { stroke: string; dot: string }) {
  const R = 25.5;
  const rimW = 3.6;
  const innerR = R * 0.46;
  const hubR = 5.8;
...
/** Monochrome wheel in currentColor (e.g. on the dark bg). */
export function HelmMark({ size = 24, className }: { size?: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" className={className} aria-hidden>
      <Wheel stroke="currentColor" dot="currentColor" />
    </svg>
  );
}

/** The gradient rounded-tile logo with the white wheel — matches the app icon. */
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" className={className} aria-hidden>
      ...
      <g transform="translate(10 10) scale(0.8)">
        <Wheel stroke="#ffffff" dot="#6d28d9" />
      </g>
    </svg>
  );
}
...
export function HelmLockup({
  ...
  return (
    <div className={cx("flex items-center gap-2", className)}>
      ...
      <span className="text-[19px] font-bold tracking-tight text-white">Helm</span>
      {tag && (
        <span className="rounded-full bg-white/[0.08] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white/55">
          {tag}
        </span>
      )}
    </div>
