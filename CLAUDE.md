...
- **The AI is intentionally OFF by default** (no key in `.env`) because a runaway auto-firing Opus brief once drained the budget (~$11.55 in one sitting). Turn it on only for live demos. Before deploying, verify the host (Vercel) has no stray `ANTHROPIC_API_KEY` unless intended — the `/api/agent` relay is billed to whatever key is set. The billed routes (Ask/Brief/Draft/Vision) are gated by `guardAgentRequest` (origin allowlist + optional `x-helm-key` secret + per-IP and global rate caps); on a public deploy with a key, SET `HELM_ALLOWED_ORIGINS` (see `.env.example`). The caps are best-effort/per-instance on serverless, so the hard ceiling remains the credit balance — keep auto-reload OFF. A blocked call returns `{ available:false }` so the client degrades to the rule engine (graceful degradation preserved).
